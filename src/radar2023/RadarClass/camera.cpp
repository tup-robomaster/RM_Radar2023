...
MV_Camera::MV_Camera(bool Is_init)
{
    ...
    {
        printf("[ERROR]CameraInit Failed: %d\n", this->iStatus);
        return;
    }
...
void CameraThread::adjustExposure(MV_Camera &cap)
{
    ...
    cout << "[INFO]"
         << "Setting Expoure Time " << ex << "us" << endl;
    cout << "[INFO]"
         << "Setting analog gain " << gain << endl;
...
FrameBag CameraThread::read()
{
    ...
    {
        cout << "[ERROR]"
             << "Failed to get frame!--CameraThread::read()" << endl;
        this->release();
    }
    return framebag;
}
